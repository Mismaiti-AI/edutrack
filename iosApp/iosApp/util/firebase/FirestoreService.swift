...
class IOSFirestoreService: NSObject, FirestoreService {
    ...
    func getCollection(collectionPath: String, whereField: String, equalsValue: String) async throws -> [[String: Any]] {
        let snapshot = try await db.collection(collectionPath).whereField(whereField, isEqualTo: equalsValue).getDocuments()
        return snapshot.documents.map { doc in
            var data = doc.data()
            data["id"] = doc.documentID
            return data
