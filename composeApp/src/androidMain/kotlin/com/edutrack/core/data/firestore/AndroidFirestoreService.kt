package com.edutrack.core.data.firestore
...
class AndroidFirestoreService : FirestoreService {
    ...
    override suspend fun getCollection(collectionPath: String, whereField: String, equalsValue: String): List<Map<String, Any?>> {
        val snapshot = firestore.collection(collectionPath).whereEqualTo(whereField, equalsValue).get().await()
        return snapshot.documents.mapNotNull { doc -> doc.data?.plus("id" to doc.id) }
    }
